...
public class Bishop {
	...
	static SortedMap<Integer,List<Integer>> bishopIndices = new TreeMap<>();
	static SortedMap<SquareEnum,List<SquareEnum>> bishopMoves = new TreeMap<>();
	...
	public static final List<Integer> canGoToIndices (int index){
		return bishopIndices.get(index);
	}
	
	public static final SquareEnum[] canGoToSquares (SquareEnum currentSquare){
		SquareEnum[] squareArray = (SquareEnum[]) bishopMoves.get(currentSquare).toArray();
		return squareArray;
	...
	public static final SquareEnum[] canGoToSquares (int index){
		SquareEnum[] squareArray =ChessBoard.getSquaresFromIndices(bishopIndices.get(index));
		return squareArray;
	}
	...
	public static void main(String[] args) {
		...
		for(Entry<Integer, List<Integer>> entry:bishopIndices.entrySet()){
			System.out.println("A "+PIECE_NAME+" on square "+entry.getKey()+" can possibly goto "+entry.getValue());
		}
		for(Entry<SquareEnum, List<SquareEnum>> entry:bishopMoves.entrySet()){
			System.out.println("A "+PIECE_NAME+" on square "+entry.getKey()+" can possibly goto "+entry.getValue());
		}
	...
	public static void computeIndicesAndMoves(){
		...
		for(int i=0;i<length;i++){
			for (int j = 0; j < indices.length; j++) {
				...
				bishopIndices.put(index, indices);
				...
				bishopMoves.put(squareEnum, squareEnumsList);
